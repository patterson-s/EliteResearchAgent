...
def format_labels_for_prompt(labels: List[Dict[str, Any]]) -> str:
    ...
    for label in labels:
        ...
        lines.append(f"  Organization: {label['awarding_organization']}")
        lines.append(f"  Year: {label.get('year', 'N/A')}")
    return "\n".join(lines)

def classify_award_event(
    event: Dict[str, Any],
...
def classify_award_events(
    ...
    print("\n" + "="*80)
    print("PHASE 2B: CLASSIFY AWARD EVENTS")
    print("="*80)
    ...
    prompt_template = load_prompt(script_dir / config["prompts"]["classify_award_event"])
    
    co = cohere.Client(api_key)
    
    print(f"\nClassifying {len(award_events)} award events into {len(labels)} labels...")
    
    classifications = []
    
    for i, event in enumerate(award_events, 1):
        print(f"  [{i}/{len(award_events)}] Classifying event {event['event_index']}...", end="")
        
        classification = classify_award_event(
    ...
    assigned_count = len(award_events) - none_count
    
    print(f"\nClassification complete:")
    print(f"  Assigned to labels: {assigned_count} ({assigned_count/len(award_events)*100:.1f}%)")
    print(f"  NONE: {none_count} ({none_count/len(award_events)*100:.1f}%)")
    ...
    output = {
        ...
        "summary": {
            "total_events": len(award_events),
            "assigned": assigned_count,
            "none": none_count,
            "coverage_percent": assigned_count/len(award_events)*100,
            "label_counts": label_counts
        }
    }
    
    output_file = output_dir / "02b_award_classifications.json"
